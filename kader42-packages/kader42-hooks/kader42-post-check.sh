...
if [ -f /boot/vmlinuz-linux ]; then
    ...
    echo "⚠️  [Kader42-Post-Check] Couldn't determine the file version of the kernel image!"
    FILE_VERSION="missing"
fi

# 3. Fallback: If the initramfs is missing or empty, abort immediately
if [ ! -s /boot/initramfs-linux.img ]; then
    echo "⚠️ [Kader42-Post-Check] CRITICAL: initramfs-linux.img ist korrupt oder fehlt!"
...
if [ "$FILE_VERSION" != "missing" ] && [ "$FILE_VERSION" != "corrupt" ]; then
    ...
    if [ ! -d "$MODULES_DIR" ]; then
        echo "⚠️  [Kader42-Post-Check] The module directory for version $FILE_VERSION is completely missing!"
        touch /boot/kader42-rollback.trigger
        exit 0

    elif [ ! -f "$MODULES_DIR/modules.dep" ] || [ ! -s "$MODULES_DIR/modules.dep" ]; then
        echo "⚠️  [Kader42-Post-Check] Module dependencies (modules.dep) are corrupted or empty!"
        touch /boot/kader42-rollback.trigger
        exit 0
    else
        # Determine the highest installed module version
        MODULES_VER=$(ls -1 /usr/lib/modules/ | sort -V | tail -n 1)
        echo "[Kader42-Post-Check] The kernel modules version is $MODULES_VER"    
    fi
    ...
    # Bash syntax corrected: Either use ‘||’ between two [ ] or ‘-o’ inside [ ]
    if [ "$NORM_PKG" != "$NORM_FILE" ] || [ "$NORM_FILE" != "$NORM_MODULE" ]; then
        ...
        echo "========================================================================="
        echo "[Kader42-Post-Check] Set the rollback flag for the next boot..."
        touch /boot/kader42-rollback.trigger
    else
